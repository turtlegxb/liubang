...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run symbol-removal ablation tests against the current backtest.")
    ...
    parser.add_argument("--max-symbols", type=int, default=None, help="Optional cap for quick smoke runs.")
    return parser
...
def main() -> int:
    ...
    tradable_symbols = tuple(symbol for symbol in symbols if symbol not in NON_TRADABLE_CONTEXT_SYMBOLS)
    if args.max_symbols is not None:
        tradable_symbols = tradable_symbols[: args.max_symbols]
    ...
    try:
        ...
        rows = []
        for removed_symbol in tradable_symbols:
            ablated_symbols = tuple(symbol for symbol in symbols if symbol != removed_symbol)
            report = run_backtest(
                history_by_symbol,
            ...
            rows.append(build_row(removed_symbol, baseline, report))

        output_path = Path(args.output_dir) / f"ablation_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S_%f')}.csv"
        write_rows(output_path, rows)
    except Exception as exc:
        print(f"ablate_symbols failed: {exc}", file=sys.stderr)
        return 1

    ranked = sorted(rows, key=lambda row: row["delta_pnl"], reverse=True)
    print("Symbol ablation summary")
    ...
    for row in ranked[:5]:
        print(
            f"  -{row['removed_symbol']}: delta_pnl={row['delta_pnl']} "
            f"delta_return={row['delta_return_pct']}% trades={row['trade_count']}"
        )
...
def build_row(removed_symbol: str, baseline: dict, report: dict) -> dict:
